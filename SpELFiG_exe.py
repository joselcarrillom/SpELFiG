...
def filter_lines_by_presence(spec, linelist, window_width=20, min_snr_threshold=5.0):
	filtered_linelist = []
...
def add_components_bybic(linelist0, dfparams, continuum, n_extra_max, LINES_EML, max_width,
	f_sigma):
...
# Produce the parameter estimations via Scipy in case is needed: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
linelist_0 = create_linelist(ALL, specrange0, width_NLR, MAXFLUX, NCOMP0)

# Examine the presence of the different emission lines:
linelist_1 = filter_lines_by_presence(SPEC, linelist_0)

if (not os.path.exists(PRIOR_FILE)) | (REWRITE_FLAG is True):
	# First fit to estimate the parameters of the first component:
	print(' >>>>>>>   STEP ONE: Determining 1st component of emission lines  <<<<<<<')
	...
	SPECFIT2_BLR = add_components_bybic(linelist_1, dfresults_1, continuum, N_EXTRA_BLR,
	...
	SPECFIT2_OUT = add_components_bybic(linelist_2, dfresults_2, continuum, N_EXTRA_OUT,
	...
	# In case this block is executed produce the plot with the sepctral model of thi parameters AT
	# THIS STAGE:
	# These are commmented to save space:
	# SPECFIT_PRE_MCMC = specfitplot(SPECFIT2_OUT, scipy=True)
	# SPECFIT_PRE_MCMC.savefig(outputname+f'PRIORS_spec_plot.png', format='png')
...
if config.properties_file_flag is True:
	SPECFIT_FINAL.lines_properties_df.to_csv(outputname + '_line_properties.csv', index=False)
